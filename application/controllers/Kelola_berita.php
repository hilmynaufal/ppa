...
class Kelola_berita extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            ...
            $config['first_url'] = base_url() . 'index.php/kelola_berita/index.html?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'index.php/kelola_berita/index/';
            $config['first_url'] = base_url() . 'index.php/kelola_berita/index/';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = FALSE;
        $config['total_rows'] = $this->Model_berita->total_rows($q);
        $kelola_berita = $this->Model_berita->get_limit_data($config['per_page'], $start, $q);
    ...
    {
        $row = $this->Model_berita->get_by_id($id);
        if ($row) {
            $data = array(
		...
		'user_id' => $row->user_id,
            ...
            $this->template->load('template','kelola_berita/tbl_ppa_berita_acara_read', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('kelola_berita'));
        }
    }
...
//
        ...
        $this->data = array(
            ...
            'start' => $start,
                 'button' => 'Create',
            'action' => site_url('kelola_berita/create_action'),
...
//       
//	);
        $this->template->load('template','kelola_berita/tbl_ppa_berita_acara_form', $this->data);
    }
...
//
        ...
        //$this->data['kelola_data_pelapor'] = $kelola_data_pelapor;
        ...
        if ($row) {
            $this->data = array(
               ...
               'kelola_data_pelapor'=> $kelola_pelapor,
                'button' => 'Update',
                'action' => site_url('kelola_berita/update_action'),
		...
		'pelapor_nik' => set_value('pelapor_nik', $row->pelapor_nik),
                'pelapor_idusers' => set_value('pelapor_nik', $row->pelapor_idusers),
		...
		'korban_tgl_lahir' => set_value('korban_tgl_lahir', $row->korban_tgl_lahir),
                'korban_prop' => set_value('korban_desa', $row->korban_prop),
                'korban_kab' => set_value('korban_desa', $row->korban_kab),
		...
		'user_id' => set_value('user_id', $row->user_id),
              //  'button_aktif' => set_value('button_aktif','200'),
	    );
    ...
    public function update_action() 
    {
		...
		'user_id' => $_SESSION['user_id'],
            ...
            $this->Model_berita->update($this->input->post('berita_acara_id', TRUE), $data);
...
//        
	...
	$this->form_validation->set_rules('korban_tempat', 'Lokasi Kejadian', 'trim|required');
...
//	$this->form_validation->set_rules('korban_kec', 'korban kec', 'trim|required');
     //   $this->form_validation->set_rules('korban_email', 'korban email', 'trim|valid_email');
...
//	$this->form_validation->set_rules('berita_acara_id', 'berita_acara_id', 'trim');
	...
	foreach ($this->Model_berita->get_all() as $data) {
            $kolombody = 0;
...
/* Location: ./application/controllers/Kelola_berita.php */
