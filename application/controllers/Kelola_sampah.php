...
class Kelola_sampah extends CI_Controller
{
    ...
    {
        $this->template->load('template','kelola_sampah/tbl_ppa_berita_acara_log_list');
    } 
    
    public function json() {
        header('Content-Type: application/json');
        echo $this->Model_sampah->json();
    }

    public function read($id) 
    {
        $row = $this->Model_sampah->get_by_id($id);
        if ($row) {
            $data = array(
		...
		'pelapor_kab' => $row->pelapor_kab,
		'pelapor_kec' => $row->pelapor_kec,
		'pelapor_desa' => $row->pelapor_desa,
		...
		'korban_kab' => $row->korban_kab,
		'korban_kec' => $row->korban_kec,
		'korban_desa' => $row->korban_desa,
		...
		'pelaku_kab' => $row->pelaku_kab,
		'pelaku_kec' => $row->pelaku_kec,
		'pelaku_desa' => $row->pelaku_desa,
		...
		'user_id' => $row->user_id,
            ...
            $this->template->load('template','kelola_sampah/tbl_ppa_berita_acara_log_read', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('kelola_sampah'));
        }
    }
    ...
    public function create() 
    {
        $data = array(
            'button' => 'Create',
            'action' => site_url('kelola_sampah/create_action'),
        ...
        $this->template->load('template','kelola_sampah/tbl_ppa_berita_acara_log_form', $data);
    }
    ...
    public function create_action() 
    {
		...
		'korban_usia' => $this->input->post('korban_usia',TRUE),
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('kelola_sampah/update_action'),
		...
		'korban_usia' => set_value('korban_usia', $row->korban_usia),
    ...
    public function update_action() 
    {
		...
		'user_id' => $this->input->post('user_id',TRUE),
            ...
            $this->Model_sampah->update($this->input->post('berita_acara_id', TRUE), $data);
	...
	$this->form_validation->set_rules('korban_usia', 'korban usia', 'trim|required');
    ...
    public function excel()
    {
        ...
        $namaFile = "tbl_ppa_berita_acara_log.xls";
        $judul = "tbl_ppa_berita_acara_log";
        $tablehead = 0;
        $tablebody = 1;
        $nourut = 1;
        //penulisan header
	...
	xlsWriteLabel($tablehead, $kolomhead++, "Korban Usia");
	...
	foreach ($this->Model_sampah->get_all() as $data) {
            $kolombody = 0;
	    ...
	    xlsWriteNumber($tablebody, $kolombody++, $data->korban_usia);
...
/* End of file Kelola_sampah.php */
/* Location: ./application/controllers/Kelola_sampah.php */
