...
class Kelola_laporan extends CI_Controller
{
	...
	public function index()
	{
		...
		if ($q <> '') {
			...
			$config['first_url'] = base_url() . 'index.php/kelola_laporan/index.html?q=' . urlencode($q);
		} else {
			$config['base_url'] = base_url() . 'index.php/kelola_laporan/index/';
			$config['first_url'] = base_url() . 'index.php/kelola_laporan/index/';
		}

		$config['per_page'] = 10;
		$config['page_query_string'] = FALSE;
		$config['total_rows'] = $this->Model_laporan->total_rows($q);
		$kelola_laporan = $this->Model_laporan->get_limit_data($config['per_page'], $start, $q);
		...
		$data = array(
			'kelola_laporan_data' => $kelola_laporan,
			'q' => $q,
			'pagination' => $this->pagination->create_links(),
			'total_rows' => $config['total_rows'],
			'start' => $start,
			'tahun' => $tahun,
			'laporan_all' => $this->Model_laporan->get_all($tahun)
		);
		$this->template->load('template', 'kelola_laporan/tbl_ppa_berita_acara_list', $data);
	}
	...
	public function laporan_rekap()
	{
		...
		$data = array(
			'button' => 'Create',
			'action' => site_url('kelola_laporan/cetak_rekap'),

			'id_kecamatan' => set_value('id_kecamatan'),
			'id_desa' => set_value('kota_lahir'),


		);

		$this->template->load('template', 'kelola_laporan/laporan_rekap', $data);

	}


	public function laporan_batang()
	{

		$this->template->load('template', 'kelola_laporan/laporan_batang', "");

	}
	public function laporan_gis()
	{

		$this->template->load('template', 'kelola_laporan/laporan_gis', "");
	...
	public function cetak_rekap()
	{
		...
		$data['rekap'] = $this->Model_laporan->rekap_data($data_cari);

		if (!empty($data['rekap'])) {
			$this->template->load('template', 'kelola_laporan/cetak_rekap', $data);
		} else {
	...
	public function create()
	{
		$data = array(
			'button' => 'Create',
			'action' => site_url('kelola_laporan/create_action'),
		...
		$this->template->load('template', 'kelola_laporan/tbl_ppa_berita_acara_form', $data);
	}
	...
	public function create_action()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$data = array(
				...
				'berita_acara_keterangan' => $this->input->post('berita_acara_keterangan', TRUE),
				...
				'korban_foto1' => $this->input->post('korban_foto1', TRUE),
				'korban_foto2' => $this->input->post('korban_foto2', TRUE),
			...
			$this->Model_laporan->insert($data);
			$this->session->set_flashdata('message', 'Create Record Success 2');
			redirect(site_url('kelola_laporan'));
		}
	}

	public function update($id)
	{
		$row = $this->Model_laporan->get_by_id($id);

		if ($row) {
			$data = array(
				'button' => 'Update',
				'action' => site_url('kelola_laporan/update_action'),
	...
	public function update_action()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->Model_laporan->update($this->input->post('berita_acara_id', TRUE), $data);
	...
	public function _rules()
	{
		...
		$this->form_validation->set_rules('berita_acara_keterangan', 'berita acara keterangan', 'trim|required');
	...
	// 	xlsWriteLabel($tablehead, $kolomhead++, "Berita Acara Keterangan");
	...
	// 	$tahun = $this->input->get('tahun', TRUE);
	// 	foreach ($this->Model_laporan->get_all($tahun) as $data) {
	// 		$kolombody = 0;
	...
	// 		xlsWriteLabel($tablebody, $kolombody++, $data->berita_acara_keterangan);
	...
	public function excel()
	{
		// Method ini tidak digunakan lagi, diganti dengan export sisi client di view list
		/*
		$this->load->helper('exportexcel');
		$tahun = $this->input->get('tahun', TRUE);
		$data = $this->Model_laporan->get_all($tahun);
		$header = [...];
		$rows = [...];
		export_excel_ci($header, $rows, 'tbl_ppa_berita_acara.xlsx');
		*/
	}
...
/* End of file Kelola_laporan.php */
/* Location: ./application/controllers/Kelola_laporan.php */
/* Please DO NOT modify this information : */
/* Generated by Harviacode Codeigniter CRUD Generator 2023-07-17 02:53:06 */
/* http://harviacode.com */